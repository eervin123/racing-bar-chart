...
def create_animation(data_file='rwa-xyz-treasury_market_cap_timeseries.csv', save_to_gif=False, gif_filename="rwa_provider_animation.gif"):
    ...
    color_mapping.update({
        'Securitize': '#1f77b4',
        'Ondo': '#ff7f0e',
        'Franklin Templeton Benji Investments': '#2ca02c',
        'Wisdomtree Prime': '#d62728',
        'Arca Labs': '#9467bd',
        'Maple': '#8c564b',
        'TrueFi': '#e377c2',
        'Backed Finance': '#7f7f7f',
        'Open Eden': '#bcbd22',
        'Superstate': '#17becf',
        'Swarm': '#9edae5',
        'Matrixdock': '#f7b6d2',
        'Hashnote': '#c7c7c7',
        'OpenTrade (Perimeter Protocol)': '#dbdb8d',
        'Midas': '#ffbb78'
    })

    dates = data_long['Date'].unique()

    # Get the overall top 10 providers by max value to ensure consistency
    top_providers = data_long.groupby('Fund')['Value'].max().nlargest(10).index

    frames = []

    for date in dates:
        frame_df = data_long[data_long['Date'] == date]
        frame_df = frame_df[frame_df['Fund'].isin(top_providers)].sort_values(by='Value', ascending=False)
        frame_df = frame_df.set_index('Fund').reindex(top_providers).fillna(0).reset_index()
    ...
    initial_df = initial_df[initial_df['Fund'].isin(top_providers)].sort_values(by='Value', ascending=False)
    initial_df = initial_df.set_index('Fund').reindex(top_providers).fillna(0).reset_index()
    ...
    fig = go.Figure(
        ...
        layout=go.Layout(
            title="Growth of RWA Treasury Market Cap by Provider",
            xaxis=dict(title="Total Asset Value (Dollar)", range=[0, data_long['Value'].max() * 1.1]),
            yaxis=dict(title="Provider Name", showticklabels=False, categoryorder='total ascending'),
...
if __name__ == "__main__":
    ...
    data_file = 'rwa-xyz-treasury_market_cap_timeseries.csv'
    save_to_gif = True  # Change to False to display the animation on screen
    gif_filename = "rwa_provider_animation.gif"
